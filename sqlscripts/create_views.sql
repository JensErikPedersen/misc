...
SELECT CONCAT(e.event_type_prefix, ev.id) eventcode, e.name event_name, r.name room_name,  e.max_participants, CONCAT(p.first_name, " ", p.last_name) instructor, ev.event_date, ev.start_time, ev.end_time FROM event_types e
...
ORDER BY ev.event_date DESC, ev.start_time, e.name;
